...
export default function CsatPage() {
  ...
  if (error) {
    return <div className="max-w-md mx-auto mt-16 bg-white border rounded-2xl p-6 text-center text-slate-600">{error}</div>;
  }
  if (!info) return <div className="p-12 text-center text-slate-500">Loading…</div>;
  if (!info.ok || done) {
    return (
      <div className="max-w-md mx-auto mt-16 bg-white border rounded-2xl p-6 text-center">
  ...
  return (
    <div className="max-w-md mx-auto mt-16 bg-white border rounded-2xl p-6 space-y-4">
      ...
      <div className="flex gap-2 justify-center">
        {[1, 2, 3, 4, 5].map((n) => (
          <button
            ...
            type="button"
            onClick={() => submit(n)}
            className={`w-12 h-12 rounded-full border text-lg font-bold ${score === n ? 'bg-brand text-white' : 'bg-white'}`}
          >
            {n}
          </button>
        ))}
