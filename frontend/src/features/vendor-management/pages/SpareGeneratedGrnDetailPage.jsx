...
import { fetchSpareGeneratedGrnOverview, fetchSpareGrnReceivedProducts } from '../vendorManagementApi';

function formatSpoType(t) {
  if (!t) return 'Spare parts';
  return String(t)
    .replace(/_/g, ' ')
...
export default function SpareGeneratedGrnDetailPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await fetchSpareGeneratedGrnOverview(spoId);
  ...
  async function openGrnModal(grnId) {
    ...
    try {
      const { data: res } = await fetchSpareGrnReceivedProducts(spoId, grnId);
  ...
  if (!data) {
    return (
      <div className="text-sm text-slate-600 py-8">
        <Link to="/vendor-management/spare-parts-po" className="text-teal-700 font-medium">
          ← Spare parts PO
        </Link>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Laravel «view_parts_purchase_order_detail» — grouped GRNs</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              to={`/vendor-management/spare-parts-po/${spoId}/receive`}
              ...
              <div className="flex flex-col sm:flex-row sm:items-stretch sm:justify-between gap-2 sm:gap-0 bg-gradient-to-r from-rose-100 via-rose-50 to-rose-100/90 border-b border-rose-100 px-3 py-3 sm:px-4">
                ...
                <div className="flex items-center justify-center">
                  <span className="inline-flex rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 px-4 py-1.5 text-xs sm:text-sm font-semibold capitalize">
                    {formatSpoType(po.purchase_order_type)}
                  </span>
                </div>
            ...
            <div className="rounded-lg border border-slate-200 bg-white shadow-sm overflow-hidden">
              ...
              {rows > 0 ? (
                ...
                <div className="px-4 py-8 text-center text-slate-500 text-sm">
                  No GRNs recorded for this spare PO yet.
                </div>
              )}
            </div>
          </div>
              ...
              {modalLoading ? (
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {(modalPayload?.items || []).map((item) => {
                    const title = [item.brand, item.model].filter(Boolean).join(' - ') || 'Spare';
                    const specLine = [item.processor, item.ram || item.storage].filter(Boolean).join(' | ');
                    return (
                      <div
                        key={item.serial_id}
                      ...
                      >
                        ...
                        {specLine ? <p className="text-xs text-slate-600 mb-3">{specLine}</p> : null}
