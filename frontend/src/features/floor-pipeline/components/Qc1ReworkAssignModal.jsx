...
export default function Qc1ReworkAssignModal({ ticket, open, onClose, onConfirm, confirming = false }) {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[85vh] flex flex-col">
        <div className="flex items-start justify-between border-b px-4 py-3">
          <div>
            <h2 className="font-semibold text-slate-900">Assign QC1 Technician</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          <p className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg p-3">
            QC2 failed — select the QC1 inspector who will handle rework. The ticket will move to QC1 and be assigned to them.
          </p>
          {loading ? (
            <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-blue-600" /></div>
          ) : !members.length ? (
            <p className="text-sm text-slate-500 text-center py-6">No QC1 team members found</p>
          ) : (
            members.map((m) => {
        ...
        <div className="border-t p-3 space-y-2">
          ...
          >
            {confirming ? 'Moving to QC1…' : 'Assign & return to QC1'}
          </button>
