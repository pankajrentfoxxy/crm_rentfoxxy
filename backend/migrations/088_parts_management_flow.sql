...
--   Purchase -> Part Instances -> Request -> Approval ->
--   Attach -> Config Update -> Expense Tracking
-- ============================================================

-- 1. Parts catalog — richer categorisation + an updated_at column
--    (the base `parts` table only had created_at).
ALTER TABLE parts
...
-- 2. Part Instances — one row per PHYSICAL unit, unique PRT-YYYYMMDD-NNNN id.
--    part_id is the CATALOG id (parts.part_id); prt_id is the INSTANCE id.
...
-- 3. Part Requests — EXTEND the existing basic table.
--    (existing: request_id, ticket_id, requested_by, part_name, description,
--     status, created_at, updated_at). We do NOT re-declare `status` here so the
--     legacy values stay valid; the new lifecycle values are enforced in code.
...
-- Backfill request_number for any pre-existing rows.
UPDATE part_requests
   SET request_number = 'PRQ-' || LPAD(request_id::text, 4, '0')
 WHERE request_number IS NULL;

-- 4. Document sequences for PRQ numbers and PRT instance ids.
...
-- 6. Denormalised open-request counter on tickets (for fast stage gating).
ALTER TABLE tickets
  ADD COLUMN IF NOT EXISTS open_part_requests INT DEFAULT 0;

-- 7. Permission sections + default role grants.
...
VALUES
  ...
  ('technician',    'parts_requests',    true, true,  false, false),
