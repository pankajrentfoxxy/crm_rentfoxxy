...
--
-- 'guard' added 22 Sep 2026. This migration was written on a branch that is 201
-- commits behind staging, and 'guard' entered the live users_role_check after
-- that fork. One live user holds it, so the list as written dropped a role that
-- exists -- a dry run against the live schema refused the constraint. The list
-- below is the LIVE constraint read back from pg_constraint, plus the two new
-- support roles. Do not regenerate it from the old branch.
