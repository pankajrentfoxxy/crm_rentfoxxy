...
async function main() {
  const sqlPath = path.join(__dirname, '../migrations/092_grn_config_verification.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');
  await pool.query(sql);
  await pool.query(`INSERT INTO schema_migrations (name) VALUES ('092_grn_config_verification.sql') ON CONFLICT (name) DO NOTHING`);
