...
function normalizeGrade(generation) {
  ...
  if (m) {
    const n = m[1];
    const ord = ({ 1: 'st', 2: 'nd', 3: 'rd' })[n] || 'th';
    // Matrix uses "5th", "12th", etc. (even for 1/2/3 if present)
    return `${n}th`;
  }
  return raw;
}
 ...
 */
function lookupDeclaredValueForUnit(processor, generation) {
...
/**
 * Sum declared values for a list of units ({ processor, generation }).
