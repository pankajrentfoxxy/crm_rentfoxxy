 ...
 * Used to autofill GenerateWayBill Declared Value (₹).
 */
const BLUEDART_DECLARED_VALUE_MATRIX = [
...
/**
 * Resolve Apple Silicon matrix grade from processor / generation / model.
 ...
 * @param {string} [model]
 * @returns {number|null} amount for one laptop, or null if no matrix match
 */
function lookupDeclaredValueForUnit(processor, generation, model) {
...
/**
 * Sum declared values for a list of units ({ processor, generation, model / model_name }).
 * Unmatched units are skipped (contribute 0). Returns null if nothing matched.
 */
function sumDeclaredValueForUnits(units = []) {
  let total = 0;
  let matched = 0;
...
module.exports = {
  BLUEDART_DECLARED_VALUE_MATRIX,
  normalizeCategory,
  normalizeGrade,
  resolveAppleGrade,
